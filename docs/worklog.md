...
# 2026-07-17 — 신규 가입·가족 등록 안정화 및 운영 배포

## 운영 웹 최신화

- 최신 Web 빌드는 생성됐지만 `/var/www/soricall`에는 이전 정적 파일이 남아 있던 배포 누락을 확인했다.
- APK 다운로드 디렉터리를 보존하면서 최신 Web 정적 파일을 운영 경로에 재배포했다.
- 공개 HTTPS 주소가 최신 해시 자산을 반환하는지 매 배포 후 확인했다.
- 공개 `soricall.apk`와 Release 빌드 산출물의 SHA-256 일치를 재확인했다.

## 신규 가입과 계정 전환 수정

- 신규 계정으로 전환한 직후 이전 계정의 `familyId`가 남아 `family access denied`가 발생하던 문제를 수정했다.
- 가입·로그인 시 가족, 보호 대상, 확인 가족, 초대 상태를 초기화한다.
- 보호 방식 진입 시 현재 계정이 접근할 수 있는 가족 ID인지 다시 검증한다.
- 회원가입 버튼을 새로 누르면 이름, 휴대전화 인증, 비밀번호, 약관 동의와 이전 로그인 세션을 모두 초기화한다.
- 이전 세션 갱신 응답이 늦게 도착해 신규 가입 상태를 덮어쓰지 못하도록 차단했다.
- 신규 비밀번호 입력란을 브라우저 자동완성에서 기존 비밀번호와 구분했다.

## 확인 가족 등록 흐름 개선

- `내 전화를 보호받고 싶어요` 흐름을 `가족 추가 → 등록 목록 확인 → 가족 등록 완료 → 음성·얼굴 등록 안내` 순서로 변경했다.
- 실제로 추가된 확인 가족이 한 명 이상일 때만 가족 등록 완료 버튼이 활성화된다.
- 등록된 확인 가족의 이름, 휴대전화 끝자리와 등록 인원을 화면에 표시한다.
- 등록 카드에 삭제 버튼과 확인창을 추가하고 서버 데이터까지 삭제하는 API를 구현했다.
- 삭제 대상이 같은 가족의 확인 가족인지 API에서 검증하며 마지막 가족 삭제 시 완료 버튼이 다시 비활성화된다.

## 검증 및 배포

- Web production build 성공
- 확인 가족 생성·조회·삭제 API 테스트 성공
- API 전체 테스트: `41 passed, 10 failed` (휴대전화 가입 전환 후 미갱신된 기존 이메일 가입 테스트 9건, 기존 SQLite 외래키 검사 1건)
- API 컨테이너 재빌드 및 정상 기동 확인
- 공개 API health와 삭제 endpoint 노출 확인
- 공개 Web 최신 자산 응답 확인
- Android APK 다운로드 파일 보존 확인

## 운영 참고

- 현재 공개 API는 `soricall-api-dev` 컨테이너가 포트 8000에서 서비스 중이다.
- `.env.production`이 없어 production compose 기반 전환은 별도 운영 작업으로 남아 있다.
- Android 운영 키스토어와 비밀번호는 Git에 포함하지 않고 별도 보안 백업이 필요하다.
